...
class BaselineModel:
    def __init__(self, data_path, model_type, model_params, vectorizer_type, vectorizer_params, extra_feature=None):
        ...
        self.vectorizer_type = vectorizer_types.get(vectorizer_type)
        if self.vectorizer_type is None or self.model is None:
    ...
    def read_data(self):
        train_data = read_file(os.path.join(self.data_path, "train"))
        dev_data = read_file(os.path.join(self.data_path, "dev"))
        test_data = read_file(os.path.join(self.data_path, "test"))
    ...
    def build_and_compile_model(self, combination, num_tokens, num_labels):
        model = Sequential()
        model.add(Embedding(num_tokens, combination['glove_length'], embeddings_initializer=Constant(emb_matrix), trainable=combination['glove_trainable']))
        # Here you should add LSTM layers (and potentially dropout)

        multiple_lstm_layers = combination['no_of_lstm'] > 1
        model.add(LSTM(units=512, activation=combination['activation_function'], return_sequences=multiple_lstm_layers))
        model.add(Dropout(combination['dropout']))
        ...
        if multiple_lstm_layers:
            ...
            model.add(LSTM(units=512, activation=combination['activation_function']))
            model.add(Dropout(combination['dropout']))
        
        multiple_dense_layers = combination['no_of_dense'] > 1
        
        if multiple_dense_layers:
            model.add(Dense(input_dim=combination['glove_length'], units=512, activation="relu"))
            model.add(Dense(units=num_labels, activation="softmax"))
        else:
            model.add(Dense(input_dim=combination['glove_length'], units=num_labels, activation="softmax"))


        model.compile(loss=combination['loss_function'], optimizer=combination['optimizer'](learning_rate=combination['lr']), metrics=['accuracy'])
    ...
    def get_callbacks(epochs):
        ...
        reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(
            monitor='val_loss', factor=0.1, patience=int(epochs // 8), verbose=0,
            mode='auto', min_delta=0.001, cooldown=0, min_lr=0.00000001
        )
    ...
    def train_lstm_combinations(self):
        ...
        for combination in all_lstm_combinations:
            ...
            vectorizer = TextVectorization(standardize=None, output_sequence_length=combination['output_length'])
...
if __name__ == "__main__":
    ...
    for model_type, model_params in model_names:
        for vectorizer_type in ["count", "tfidf"]:
            for combination in all_combinations:
                ...
                experiment.save_results(os.path.join(args.output_path, f"{model_type}_{combination}.txt"))
    ...
    for feature in additional_features:
        ...
        model.evaluate()
        model.print_results()
        model.save_results(os.path.join(args.output_path, f"baseline_{feature}.txt"))
